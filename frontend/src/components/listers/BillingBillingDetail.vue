<template>
    <v-card outlined>
        <v-card-title>
            Billing # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <String label="InvoiceFileNm" v-model="item.invoiceFileNm" :editMode="editMode" @change="change" />
            </div>
            <div>
                <String label="InvoiceFilePath" v-model="item.invoiceFilePath" :editMode="editMode" @change="change" />
            </div>
            <div>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/billings/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
